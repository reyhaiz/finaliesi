...
<form action="code/proses/input/input_nilai.php" method="POST">
    ...
    <table id="tabel-pendaftaran">
        ...
        <tr>
            <td>
				...
				while ($isi = mysqli_fetch_array($tampilkan_isi_sql)) {
					$jumlah = $isi['jumlah'];
				?>
                <input type="text" name='kode_nilai' size="25px" maxlength="6" style="background-color:#E0DFDF"
                    value="NI-<?php echo $no + $jumlah ?>" readonly>
        ...
        <td>
        <?php
            $tampilkan_isi = "select * from guru g where g.kode_guru='$informasi'";
            $tampilkan_isi_sql = mysqli_query($connect, $tampilkan_isi);
            $isi = mysqli_fetch_array($tampilkan_isi_sql);
            // while ($isi = mysqli_fetch_array($tampilkan_isi_sql)) {
                $kode_guru = $isi['kode_guru'];
                $nama_guru = $isi['nama_guru'];
                
        ?>
        <input type="hidden" name='kode_guru' size="25px" maxlength="6" style="background-color:#E0DFDF"
            value="<?php echo $kode_guru; ?>" readonly>
        <input type="text" name='nama_guru' size="25px" maxlength="6" style="background-color:#E0DFDF"
            value="<?php echo $nama_guru; ?>" readonly>
...
where g.kode_mp = mp.kode_mp and g.kode_guru='$informasi' and g.kode_mp=sk.kode_mp;";
			...
			while ($isi = mysqli_fetch_array($tampilkan_isi_sql)) {
				...
				$nama_mp = $isi['nama_mp'];
			?>
                    <option value="<?php echo $kode_sk ?>"><?php echo $kode_sk ?> | <?php echo $nama_mp ?> -
                        <?php echo $nama_sk ?>
                        <?php
			}
				?>
        ...
        <tr>
            <td><input type="text" name="angka_nilai" size="25px" maxlength="3" placeholder="ketikkan nilai.."
                    required>&nbsp;&nbsp;&nbsp; <input class="button" type="submit" value="Simpan"></td>
        </tr>
